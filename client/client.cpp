...
void Client::run() {
    ...
    try {
        ...
        while (_keep_running) {
            ...
            int iterations_ahead = sleep_and_calc_next_it(render_time);
        }
        stop();
...
void Client::update_state_from_server() {
    // Carga las actualizaciones del estado enviadas por el servidor, siempre se dibuja la ultima enviada.
...
int Client::sleep_and_calc_next_it(uint32_t render_time_ms) {
    ...
    if (render_time_ms < frame_ms) {
        ...
        std::this_thread::sleep_for(
            std::chrono::milliseconds(frame_ms - render_time_ms));
    } else {
        ...
        if (extra_time > 0) {
            std::this_thread::sleep_for(
                std::chrono::milliseconds(frame_ms - extra_time));
        }
    }
