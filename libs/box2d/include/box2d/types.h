...
/// Task interface
/// This is prototype for a Box2D task. Your task system is expected to invoke the Box2D task with
/// these arguments. The task spans a range of the parallel-for: [startIndex, endIndex) The worker
/// index must correctly identify each worker in the user thread pool, expected in [0, workerCount).
/// A worker must only exist on only one thread at a time and is analogous to the thread index.
/// The task context is the context pointer sent from Box2D when it is enqueued.
/// The startIndex and endIndex are expected in the range [0, itemCount) where itemCount is the
/// argument to b2EnqueueTaskCallback below. Box2D expects startIndex < endIndex and will execute a
/// loop like this:
///
/// @code{.c}
...
/// These functions can be provided to Box2D to invoke a task system. These are designed to work
/// well with enkiTS. Returns a pointer to the user's task object. May be nullptr. A nullptr
/// indicates to Box2D that the work was executed serially within the callback and there is no need
/// to call b2FinishTaskCallback. The itemCount is the number of Box2D work items that are to be
/// partitioned among workers by the user's task system. This is essentially a parallel-for. The
/// minRange parameter is a suggestion of the minimum number of items to assign per worker to reduce
/// overhead. For example, suppose the task is small and that itemCount is 16. A minRange of 8
/// suggests that your task system should split the work items among just two workers, even if you
/// have more available. In general the range [startIndex, endIndex) send to b2TaskCallback should
/// obey: endIndex - startIndex >= minRange The exception of course is when itemCount < minRange.
/// @ingroup world
typedef void* b2EnqueueTaskCallback(b2TaskCallback* task, int itemCount, int minRange,
...
/// Optional friction mixing callback. This intentionally provides no context objects because this
/// is called from a worker thread.
/// @warning This function should not attempt to modify Box2D state or user application state.
/// @ingroup world
typedef float b2FrictionCallback(float frictionA, uint64_t userMaterialIdA, float frictionB,
                                 uint64_t userMaterialIdB);

/// Optional restitution mixing callback. This intentionally provides no context objects because
/// this is called from a worker thread.
/// @warning This function should not attempt to modify Box2D state or user application state.
/// @ingroup world
typedef float b2RestitutionCallback(float restitutionA, uint64_t userMaterialIdA,
                                    float restitutionB, uint64_t userMaterialIdB);

/// Result from b2World_RayCastClosest
/// If there is initial overlap the fraction and normal will be zero while the point is an arbitrary
...
typedef struct b2WorldDef {
    ...
    /// Contact stiffness. Cycles per second. Increasing this increases the speed of overlap
    /// recovery, but can introduce jitter.
    float contactHertz;

    /// Contact bounciness. Non-dimensional. You can speed up overlap recovery by decreasing this
    /// with the trade-off that overlap resolution becomes more energetic.
    ...
    /// performance cores and accessing a single L2 cache. Efficiency cores and hyper-threading
    /// provide little benefit and may even harm performance.
    /// @note Box2D does not create threads. This is the number of threads your applications has
    /// created that you are allocating to b2World_Step.
    /// @warning Do not modify the default value unless you are also providing a task system and
    /// providing task callbacks (enqueueTask and finishTask).
    int workerCount;
...
/// Each body is one of these three types. The type determines how the body behaves in the
/// simulation.
/// @ingroup body
...
typedef struct b2BodyDef {
    ...
    /// @note Creating bodies at the origin and then moving them nearly doubles the cost of body
...
typedef struct b2Filter {
    ...
    /// ragdoll self-collision. In this case you would give each ragdoll a unique negative group
    /// index and apply that group index to all shapes on the ragdoll.
    int groupIndex;
} b2Filter;
...
typedef struct b2ShapeDef {
    ...
    /// Enable custom filtering. Only one of the two shapes needs to enable custom filtering. See
    /// b2WorldDef.
    ...
    /// This is flag is ignored for dynamic and kinematic shapes which always invoke contact
    /// creation.
...
/// Used to create a chain of line segments. This is designed to eliminate ghost collisions with
/// some limitations.
...
/// - you may overlap two open chains on their first three and/or last three points to get smooth
/// collision
/// - a chain shape creates multiple line segment shapes on the body
/// https://en.wikipedia.org/wiki/Polygonal_chain
/// Must be initialized using b2DefaultChainDef().
/// @warning Do not use chain shapes unless you understand the limitations. This is an advanced
/// feature.
/// @ingroup shape
 ...
 *
 * With event arrays, you can scan the events in a loop and modify the world. However, you need to
 * be careful that some event data may become invalid. There are several samples that show how to do
 * this safely.
 *
 * @{
 */
...
typedef struct b2ContactBeginTouchEvent {
    ...
    /// The transient contact id. This contact maybe destroyed automatically when the world is
    /// modified or simulated. Used b2Contact_IsValid before using this id.
...
/// A hit touch event is generated when two shapes collide with a speed faster than the hit speed
/// threshold. This may be reported for speculative contacts that have a confirmed impulse.
...
/// calling functions such as b2Body_GetTransform() because this data is delivered as a contiguous
...
/// @warning Do not attempt to modify the world inside this callback
/// @ingroup world
typedef bool b2PreSolveFcn(b2ShapeId shapeIdA, b2ShapeId shapeIdB, b2Vec2 point, b2Vec2 normal,
                           void* context);
...
/// @param normal the normal vector at the point of intersection, zero for a shape cast with initial
/// overlap
/// @param fraction the fraction along the ray at the point of intersection, zero for a shape cast
/// with initial overlap
/// @param context the user context
/// @return -1 to filter, 0 to terminate, fraction to clip the ray for closest hit, 1 to continue
/// @see b2World_CastRay
/// @ingroup world
typedef float b2CastResultFcn(b2ShapeId shapeId, b2Vec2 point, b2Vec2 normal, float fraction,
                              void* context);
...
typedef struct b2DebugDraw {
    ...
    void (*DrawSolidPolygonFcn)(b2Transform transform, const b2Vec2* vertices, int vertexCount,
