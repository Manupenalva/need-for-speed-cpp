...
static inline int b2PopCount64(uint64_t block) { return (int)__popcnt64(block); }
#else

static inline uint32_t b2CTZ32(uint32_t block) { return __builtin_ctz(block); }

static inline uint32_t b2CLZ32(uint32_t value) { return __builtin_clz(value); }

static inline uint32_t b2CTZ64(uint64_t block) { return __builtin_ctzll(block); }

static inline int b2PopCount64(uint64_t block) { return __builtin_popcountll(block); }
#endif

static inline bool b2IsPowerOf2(int x) { return (x & (x - 1)) == 0; }
