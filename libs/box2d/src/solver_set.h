...
typedef struct b2SolverSet {
    ...
    // The awake set has an array of islands. Sleeping sets normally have a single islands. However,
    // joints created between sleeping sets causes the sets to merge, leaving them with multiple
    // islands. These sleeping islands will be naturally merged with the set is woken. The static
    // and disabled sets have no islands. Islands live in the solver sets to limit the number of
    // islands that need to be considered for sleeping.
    b2IslandSimArray islandSims;

    // Aligns with b2World::solverSetIdPool. Used to create a stable id for
...
void b2TransferJoint(b2World* world, b2SolverSet* targetSet, b2SolverSet* sourceSet,
                     b2Joint* joint);
