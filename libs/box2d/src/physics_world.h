...
typedef struct b2World {
    ...
    // This is a sparse array that maps contact ids to the contact data stored in the constraint
    // graph or in the solver sets.
    ...
    // - when a body comes to rest and its sleep timer trips, I can look at the island and flag it
    // for splitting
    //   if it has removed constraints
    // - islands that have removed constraints must be put split first because I don't want to wake
    // bodies incorrectly
    // - otherwise I can use the awake islands that have bodies wanting to sleep as the splitting
    // candidates
