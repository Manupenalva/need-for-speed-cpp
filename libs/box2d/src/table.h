...
#define B2_SHAPE_PAIR_KEY(K1, K2) \
    K1 < K2 ? (uint64_t)K1 << 32 | (uint64_t)K2 : (uint64_t)K2 << 32 | (uint64_t)K1
...
typedef struct b2SetItem {
    ...
    // Update: looks like I store this to make growing the table faster, however this is wasteful
    // once the table has hit the high water mark
    // uint32_t hash;
} b2SetItem;
...
static inline int b2GetSetCount(b2HashSet* set) { return set->count; }

static inline int b2GetSetCapacity(b2HashSet* set) { return set->capacity; }
