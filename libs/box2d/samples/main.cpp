...
static int MyAllocHook(int allocType, void* userData, size_t size, int blockType,
...
static void CreateUI(GLFWwindow* window, const char* glslVersion) {
    ...
    if (file != nullptr) {
        ...
        printf("\n\nERROR: the Box2D samples working directory must be the top level Box2D "
               "directory (same as README.md)\n\n");
        exit(EXIT_FAILURE);
    }
}
...
static void UpdateUI() {
    ...
    if (s_context.showUI) {
        ImGui::SetNextWindowPos(
                {s_context.camera.width - menuWidth - 0.5f * fontSize, 0.5f * fontSize});
        ImGui::SetNextWindowSize({menuWidth, s_context.camera.height - fontSize});

        ImGui::Begin(
                "Tools", &s_context.showUI,
            ...
            if (ImGui::BeginTabItem("Samples")) {
                ...
                while (i < g_sampleCount) {
                    bool categorySelected =
                            strcmp(category, g_sampleEntries[s_context.sampleIndex].category) == 0;
...
// {,,ucrtbased.dll}_crtBreakAlloc = <allocation number>
    ...
    if (fullscreen) {
        ...
        s_context.window = glfwCreateWindow(int(s_context.camera.width),
                                            int(s_context.camera.height), buffer, nullptr, nullptr);
    }
    ...
    while (!glfwWindowShouldClose(s_context.window)) {
        ...
        // ImGui_ImplGlfw_CursorPosCallback( s_context.window, cursorPosX / s_windowScale,
        ...
        DrawScreenString(s_context.draw, 5.0f, s_context.camera.height - 10.0f, b2_colorSeaGreen,
                         "%.1f ms - step %d - camera (%g, %g, %g)", 1000.0f * frameTime,
                         s_sample->m_stepCount, s_context.camera.center.x,
