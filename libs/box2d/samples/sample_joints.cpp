...
public:
    ...
    explicit DistanceJoint(SampleContext* context): Sample(context) {
    ...
    void UpdateGui() override {
        ...
        ImGui::SetNextWindowPos({0.5f * fontSize, m_camera->height - height - 2.0f * fontSize},
                                ImGuiCond_Once);
        ...
        if (m_enableSpring) {
            if (ImGui::SliderFloat("Tension", &m_tensionForce, 0.0f, 4000.0f)) {
                for (int i = 0; i < m_count; ++i) {
                    b2DistanceJoint_SetSpringForceRange(m_jointIds[i], -m_tensionForce,
                                                        m_compressionForce);
                    b2Joint_WakeBodies(m_jointIds[i]);
                }
            }
    ...
    static Sample* Create(SampleContext* context) { return new DistanceJoint(context); }
...
public:
    explicit MotorJoint(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void UpdateGui() override {
        ...
        ImGui::SetNextWindowPos(
                ImVec2(0.5f * fontSize, m_camera->height - height - 2.0f * fontSize),
                ImGuiCond_Once);
    ...
    static Sample* Create(SampleContext* context) { return new MotorJoint(context); }
...
public:
    explicit TopDownFriction(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 7.0f};
            m_context->camera.zoom = 25.0f * 0.4f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new TopDownFriction(context); }
};

static int sampleTopDownFriction =
        RegisterSample("Joints", "Top Down Friction", TopDownFriction::Create);

// This sample shows how to use a filter joint to prevent collision between two bodies.
// This is more specific than shape filters. It also shows that sleeping is coupled by the filter
// joint.
class FilterJoint: public Sample {
public:
    explicit FilterJoint(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 7.0f};
            m_context->camera.zoom = 25.0f * 0.4f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new FilterJoint(context); }
};
...
public:
    explicit RevoluteJoint(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new RevoluteJoint(context); }
...
public:
    explicit PrismaticJoint(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new PrismaticJoint(context); }
...
public:
    explicit WheelJoint(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new WheelJoint(context); }
...
public:
    explicit Bridge(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void UpdateGui() override {
        ...
        bool updateFriction =
                ImGui::SliderFloat("Joint Friction", &m_frictionTorque, 0.0f, 10000.0f, "%2.f");
        if (updateFriction) {
            for (int i = 0; i <= m_count; ++i) {
        ...
        if (ImGui::SliderFloat("Constraint hertz", &m_constraintHertz, 15.0f, 240.0f, "%.0f")) {
            for (int i = 0; i <= m_count; ++i) {
                b2Joint_SetConstraintTuning(m_jointIds[i], m_constraintHertz,
                                            m_constraintDampingRatio);
            }
        }

        if (ImGui::SliderFloat("Constraint damping", &m_constraintDampingRatio, 0.0f, 10.0f,
                               "%.1f")) {
            for (int i = 0; i <= m_count; ++i) {
                b2Joint_SetConstraintTuning(m_jointIds[i], m_constraintHertz,
                                            m_constraintDampingRatio);
    ...
    static Sample* Create(SampleContext* context) { return new Bridge(context); }
...
public:
    explicit BallAndChain(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, -8.0f};
    ...
    void UpdateGui() override {
        ...
        bool updateFriction =
                ImGui::SliderFloat("Joint Friction", &m_frictionTorque, 0.0f, 1000.0f, "%2.f");
        if (updateFriction) {
            for (int i = 0; i <= m_count; ++i) {
                b2RevoluteJoint_SetMaxMotorTorque(m_jointIds[i], m_frictionTorque);
            }
    ...
    static Sample* Create(SampleContext* context) { return new BallAndChain(context); }
...
// This sample shows the limitations of an iterative solver. The cantilever sags even though the
// weld joint is stiff as possible.
class Cantilever: public Sample {
public:
    enum { e_count = 8 };

    explicit Cantilever(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void UpdateGui() override {
        ...
        if (ImGui::SliderFloat("Linear Damping Ratio", &m_linearDampingRatio, 0.0f, 10.0f,
                               "%.1f")) {
            for (int i = 0; i < e_count; ++i) {
        ...
        if (ImGui::SliderFloat("Angular Damping Ratio", &m_angularDampingRatio, 0.0f, 10.0f,
                               "%.1f")) {
            for (int i = 0; i < e_count; ++i) {
    ...
    static Sample* Create(SampleContext* context) { return new Cantilever(context); }
...
public:
    ...
    explicit MotionLocks(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 8.0f};
            m_context->camera.zoom = 25.0f * 0.7f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new MotionLocks(context); }
...
public:
    ...
    explicit BreakableJoint(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 8.0f};
            m_context->camera.zoom = 25.0f * 0.7f;
        }
    ...
    void Step() override {
        for (int i = 0; i < e_count; ++i) {
            ...
            if (b2LengthSquared(force) > m_breakForce * m_breakForce) {
                ...
                DrawWorldString(m_draw, m_camera, localFrame.p, b2_colorWhite, "(%.1f, %.1f)",
                                force.x, force.y);
            }
        }
    ...
    static Sample* Create(SampleContext* context) { return new BreakableJoint(context); }
...
public:
    ...
    explicit JointSeparation(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 8.0f};
    ...
    void Step() override {
        for (int i = 0; i < e_count; ++i) {
            ...
            b2Transform localFrame = b2Joint_GetLocalFrameA(m_jointIds[i]);
            DrawWorldString(m_draw, m_camera, localFrame.p, b2_colorWhite, "%.2f m, %.1f deg",
                            linear, 180.0f * angular / B2_PI);
        }

        Sample::Step();
    }

    static Sample* Create(SampleContext* context) { return new JointSeparation(context); }
...
public:
    explicit UserConstraint(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new UserConstraint(context); }

    b2BodyId m_bodyId;
    float m_impulses[2];
};

static int sampleUserConstraintIndex =
        RegisterSample("Joints", "User Constraint", UserConstraint::Create);

// This is a fun demo that shows off the wheel joint
class Driving: public Sample {
public:
    explicit Driving(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new Driving(context); }
...
public:
    explicit Ragdoll(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 12.0f};
    ...
    void Spawn() {
        CreateHuman(&m_human, m_worldId, {0.0f, 25.0f}, 1.0f, m_jointFrictionTorque, m_jointHertz,
                    m_jointDampingRatio, 1, nullptr, false);
    ...
    static Sample* Create(SampleContext* context) { return new Ragdoll(context); }
...
public:
    explicit SoftBody(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new SoftBody(context); }
...
public:
    explicit DoohickeyFarm(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 5.0f};
            m_context->camera.zoom = 25.0f * 0.35f;
        }
    ...
    void Step() override { Sample::Step(); }

    static Sample* Create(SampleContext* context) { return new DoohickeyFarm(context); }
};
...
public:
    explicit ScissorLift(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new ScissorLift(context); }
...
public:
    explicit GearLift(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
        ...
        b2BodyId groundId;
        {
            ...
            const char* path = "m 63.500002,201.08333 103.187498,0 1e-5,-37.04166 h -2.64584 l "
                               "0,34.39583 h -42.33333 v -2.64583 l "
                               "-2.64584,-1e-5 v -2.64583 h -2.64583 v -2.64584 h -2.64584 v "
                               "-2.64583 H 111.125 v -2.64583 h -2.64583 v "
                               "-2.64583 h -2.64583 v -2.64584 l -2.64584,1e-5 v -2.64583 l "
                               "-2.64583,-1e-5 V 174.625 h -2.645834 v -2.64584 l "
                               "-2.645833,1e-5 v -2.64584 H 92.60417 v -2.64583 h -2.645834 v "
                               "-2.64583 l -26.458334,0 0,37.04166";
        ...
        b2Vec2 linkAttachPosition =
                gearPosition2 + b2Vec2{gearRadius + 2.0f * toothHalfWidth + toothRadius, 0.0f};
        b2Vec2 doorPosition = linkAttachPosition -
                              b2Vec2{0.0f, 2.0f * linkCount * linkHalfLength + doorHalfHeight};

        {
            ...
            for (int i = 0; i < count; ++i) {
                b2Polygon tooth = b2MakeOffsetRoundedBox(toothHalfWidth, toothHalfHeight, center,
                                                         rotation, toothRadius);
...

        {
            ...
            b2HexColor colors[5] = {
                    b2_colorGray,           b2_colorGainsboro, b2_colorLightGray,
                    b2_colorLightSlateGray, b2_colorDarkGray,
            };
    ...
    void UpdateGui() override {
        float fontSize = ImGui::GetFontSize();
        float height = 120.0f;
        ImGui::SetNextWindowPos(ImVec2(0.5f * fontSize, m_camera->height - height - 25.0f),
                                ImGuiCond_Once);
        ImGui::SetNextWindowSize(ImVec2(240.0f, height));
    ...
    static Sample* Create(SampleContext* context) { return new GearLift(context); }
...
public:
    explicit Door(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 0.0f};
    ...
    static Sample* Create(SampleContext* context) { return new Door(context); }
...
public:
    explicit ScaleRagdoll(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void Spawn() {
        ...
        CreateHuman(&m_human, m_worldId, {0.0f, 5.0f}, m_scale, jointFrictionTorque, jointHertz,
    ...
    void UpdateGui() override {
        ...
        if (ImGui::SliderFloat("Scale", &m_scale, 0.1f, 10.0f, "%3.2f",
    ...
    static Sample* Create(SampleContext* context) { return new ScaleRagdoll(context); }
