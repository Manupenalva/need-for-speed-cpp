...
public:
    ...
    explicit BenchmarkBarrel(SampleContext* context): Sample(context) {
...

        {
            ...
            for (int i = 0; i < 81; ++i) {
                b2Polygon box =
                        b2MakeOffsetBox(0.5f * gridSize, 0.5f * gridSize, {x, y}, b2Rot_identity);
        ...
        for (int i = 0; i < m_columnCount; ++i) {
            ...
            for (int j = 0; j < m_rowCount; ++j) {
                ...
                if (m_shapeType == e_circleShape) {
                    ...
                    CreateHuman(m_humans + index, m_worldId, bodyDef.position, scale, jointFriction,
                                jointHertz, jointDamping, index + 1, nullptr, false);
                }
    ...
    void UpdateGui() override {
        ...
        changed =
                changed || ImGui::Combo("Shape", &shapeType, shapeTypes, IM_ARRAYSIZE(shapeTypes));
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkBarrel(context); }
...
public:
    explicit BenchmarkBarrel24(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {8.0f, 53.0f};
            m_context->camera.zoom = 25.0f * 2.35f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkBarrel24(context); }
};
...
public:
    explicit BenchmarkTumbler(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkTumbler(context); }
};
...
public:
    explicit BenchmarkWasher(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {1.5f, 10.0f};
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkWasher(context); }
};
...
public:
    explicit BenchmarkManyTumblers(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkManyTumblers(context); }
...
static int benchmarkManyTumblers =
        RegisterSample("Benchmark", "Many Tumblers", BenchmarkManyTumblers::Create);

class BenchmarkLargePyramid: public Sample {
public:
    explicit BenchmarkLargePyramid(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkLargePyramid(context); }
};

static int benchmarkLargePyramid =
        RegisterSample("Benchmark", "Large Pyramid", BenchmarkLargePyramid::Create);

class BenchmarkManyPyramids: public Sample {
public:
    explicit BenchmarkManyPyramids(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkManyPyramids(context); }
};

static int benchmarkManyPyramids =
        RegisterSample("Benchmark", "Many Pyramids", BenchmarkManyPyramids::Create);

class BenchmarkCreateDestroy: public Sample {
public:
    enum { e_maxBaseCount = 100, e_maxBodyCount = e_maxBaseCount * (e_maxBaseCount + 1) / 2 };

    explicit BenchmarkCreateDestroy(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 50.0f};
            m_context->camera.zoom = 25.0f * 2.2f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkCreateDestroy(context); }
...
static int benchmarkCreateDestroy =
        RegisterSample("Benchmark", "CreateDestroy", BenchmarkCreateDestroy::Create);

class BenchmarkSleep: public Sample {
public:
    enum { e_maxBaseCount = 100, e_maxBodyCount = e_maxBaseCount * (e_maxBaseCount + 1) / 2 };

    explicit BenchmarkSleep(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 50.0f};
            m_context->camera.zoom = 25.0f * 2.2f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkSleep(context); }
...
public:
    explicit BenchmarkJointGrid(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkJointGrid(context); }
};

static int benchmarkJointGridIndex =
        RegisterSample("Benchmark", "Joint Grid", BenchmarkJointGrid::Create);

class BenchmarkSmash: public Sample {
public:
    explicit BenchmarkSmash(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkSmash(context); }
};
...
public:
    explicit BenchmarkCompound(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
...
#endif
        {
            ...
            for (int i = 0; i < height; ++i) {
                ...
                for (int j = i; j < width; ++j) {
                    float x = grid * j;
                    b2Polygon square =
                            b2MakeOffsetBox(0.5f * grid, 0.5f * grid, {x, y}, b2Rot_identity);
                ...
                for (int n = 0; n < count; ++n) {
                    ...
                    for (int i = 0; i < span; ++i) {
                        ...
                        for (int j = 0; j < span; ++j) {
                            float x = j * grid;
                            b2Polygon square = b2MakeOffsetBox(0.5f * grid, 0.5f * grid, {x, y},
                                                               b2Rot_identity);
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkCompound(context); }
};
...
public:
    explicit BenchmarkKinematic(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkKinematic(context); }
};
...
public:
    explicit BenchmarkCast(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static float CastCallback(b2ShapeId shapeId, b2Vec2 point, b2Vec2 normal, float fraction,
                              void* context) {
    ...
    void Step() override {
        ...
        if (m_queryType == e_rayCast) {
            ...
            for (int i = 0; i < sampleCount; ++i) {
                ...
                CastResult result;
                b2TreeStats traversalResult = b2World_CastShape(m_worldId, &proxy, translation,
                                                                filter, CastCallback, &result);
            ...
            for (int i = 0; i < sampleCount; ++i) {
                ...
                b2TreeStats traversalResult =
                        b2World_OverlapAABB(m_worldId, aabb, filter, OverlapCallback, &result);
        ...
        DrawTextLine("hit count = %d, node visits = %d, leaf visits = %d", hitCount, nodeVisits,
                     leafVisits);
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkCast(context); }
...
public:
    explicit BenchmarkSpinner(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkSpinner(context); }
};
...
public:
    explicit BenchmarkRain(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkRain(context); }
};
...
public:
    explicit BenchmarkShapeDistance(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 0.0f};
    ...
    void Step() override {
        if (m_context->pause == false || m_context->singleStep == true) {
            ...
            DrawTextLine("min ms = %g, ave us = %g", m_minMilliseconds,
                         1000.0f * m_minMilliseconds / float(m_count));
        ...
        DrawSolidPolygon(m_context->draw, xfA, m_polygonA.vertices, m_polygonA.count,
                         m_polygonA.radius, b2_colorBox2DGreen);
        DrawSolidPolygon(m_context->draw, xfB, m_polygonB.vertices, m_polygonB.count,
                         m_polygonB.radius, b2_colorBox2DBlue);
        ...
        DrawLine(m_context->draw, output.pointA, output.pointA + 0.5f * output.normal,
                 b2_colorYellow);
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkShapeDistance(context); }
...
static int benchmarkShapeDistance =
        RegisterSample("Benchmark", "Shape Distance", BenchmarkShapeDistance::Create);
...
public:
    explicit BenchmarkSensor(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void Step() override {
        ...
        for (int i = 0; i < events.beginCount; ++i) {
            ...
            ShapeUserData* userData =
                    static_cast<ShapeUserData*>(b2Shape_GetUserData(event->sensorShapeId));
            ...
            if (userData->active) {
                ...
                b2SurfaceMaterial surfaceMaterial =
                        b2Shape_GetSurfaceMaterial(event->visitorShapeId);
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkSensor(context); }
...
// This benchmark pushes Box2D to the limit for a large pile. It terminates once simulation is
// deemed to be slow. The higher the body count achieved, the better. Note: this benchmark stresses
// the sleep system more than any other benchmark. Better results are achieved if sleeping is
// disabled.
class BenchmarkCapacity: public Sample {
public:
    explicit BenchmarkCapacity(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new BenchmarkCapacity(context); }
