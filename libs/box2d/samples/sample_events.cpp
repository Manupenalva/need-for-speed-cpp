...
public:
    enum { e_donut = 1, e_human = 2, e_count = 32 };

    explicit SensorFunnel(SampleContext* context): Sample(context) {
...

        {
            ...
            // b2Vec2 points[] = {
            //{42.333, 44.979},	{177.271, 44.979},	{177.271, 100.542}, {142.875, 121.708},
            //{177.271, 121.708}, {177.271, 171.979}, {142.875, 193.146}, {177.271, 193.146},
            //{177.271, 222.250}, {124.354, 261.938}, {124.354, 293.688}, {95.250, 293.688},
            //{95.250, 261.938},	{42.333, 222.250},	{42.333, 193.146}, {76.729, 193.146},
            //{42.333, 171.979},	{42.333, 121.708},	{76.729, 121.708},	{42.333, 100.542},
            //};

            b2Vec2 points[] = {
                    {-16.8672504, 31.088623},    {16.8672485, 31.088623},
                    {16.8672485, 17.1978741},    {8.26824951, 11.906374},
                    {16.8672485, 11.906374},     {16.8672485, -0.661376953},
                    {8.26824951, -5.953125},     {16.8672485, -5.953125},
                    {16.8672485, -13.229126},    {3.63799858, -23.151123},
                    {3.63799858, -31.088623},    {-3.63800049, -31.088623},
                    {-3.63800049, -23.151123},   {-16.8672504, -13.229126},
                    {-16.8672504, -5.953125},    {-8.26825142, -5.953125},
                    {-16.8672504, -0.661376953}, {-16.8672504, 11.906374},
    ...
    void CreateElement() {
        ...
        if (m_type == e_donut) {
            ...
            bool colorize = true;
            CreateHuman(human, m_worldId, center, scale, jointFriction, jointHertz, jointDamping,
                        index + 1, human, colorize);
        ...
        for (int i = 0; i < sensorEvents.beginCount; ++i) {
            ...
            if (m_type == e_donut) {
                ...
                if (donut != nullptr) {
                    ...
                    // Defer destruction to avoid double destruction and event invalidation
                    // (orphaned shape ids)
    ...
    static Sample* Create(SampleContext* context) { return new SensorFunnel(context); }
...
public:
    explicit SensorBookend(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void UpdateGui() override {
        ...
        ImGui::Begin("Sensor Bookend", nullptr,
    ...
    static Sample* Create(SampleContext* context) { return new SensorBookend(context); }
...
static int sampleSensorBookendEvent =
        RegisterSample("Events", "Sensor Bookend", SensorBookend::Create);
...
public:
    ...
    explicit FootSensor(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 6.0f};
            m_context->camera.zoom = 7.5f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new FootSensor(context); }
...
public:
    ...
    explicit ContactEvent(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 0.0f};
    ...
    void Step() override {
        ...
        for (int i = 0; i < contactEvents.beginCount; ++i) {
            ...
            // The begin touch events have the contact manifolds, but the impulses are zero. This is
            // because the manifolds are gathered before the contact solver is run.

            // We can get the final contact data from the shapes. The manifold is shared by the two
            // shapes, so we just need the contact data from one of the shapes. Choose the one with
            // the smallest number of contacts.
            ...
            if (capacityA < capacityB) {
                ...
                for (int j = 0; j < countA; ++j) {
                    ...
                    if (B2_ID_EQUALS(idA, event.shapeIdB) || B2_ID_EQUALS(idB, event.shapeIdB)) {
                        assert(B2_ID_EQUALS(idA, event.shapeIdA) ||
                               B2_ID_EQUALS(idB, event.shapeIdA));
                        ...
                        for (int k = 0; k < manifold.pointCount; ++k) {
                            ...
                            DrawLine(m_draw, point.point,
                                     point.point + point.totalNormalImpulse * normal,
                                     b2_colorBlueViolet);
                ...
                for (int j = 0; j < countB; ++j) {
                    ...
                    if (B2_ID_EQUALS(idA, event.shapeIdA) || B2_ID_EQUALS(idB, event.shapeIdA)) {
                        assert(B2_ID_EQUALS(idA, event.shapeIdB) ||
                               B2_ID_EQUALS(idB, event.shapeIdB));
            ...
            if (B2_ID_EQUALS(bodyIdA, m_playerId)) {
                ...
                if (userDataB == nullptr) {
                    if (B2_ID_EQUALS(event.shapeIdA, m_coreShapeId) == false &&
                        destroyCount < e_count) {
                ...
                if (userDataA == nullptr) {
                    if (B2_ID_EQUALS(event.shapeIdB, m_coreShapeId) == false &&
                        destroyCount < e_count) {
                        // player non-core shape hit the wall
    ...
    static Sample* Create(SampleContext* context) { return new ContactEvent(context); }
...
public:
    explicit Platform(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static bool PreSolveStatic(b2ShapeId shapeIdA, b2ShapeId shapeIdB, b2Vec2 point, b2Vec2 normal,
                               void* context) {
    ...
    void UpdateGui() override {
        ...
        ImGui::Begin("One-Sided Platform", nullptr,
                     ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize);
    ...
    void Step() override {
        ...
        DrawTextLine("Platform contact count = %d, point count = %d", contactCount,
    ...
    static Sample* Create(SampleContext* context) { return new Platform(context); }
...
public:
    ...
    explicit BodyMove(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
...

        {
            ...
            b2Polygon box =
                    b2MakeOffsetBox(12.0f, 0.1f, {-10.0f, -0.1f}, b2MakeRot(-0.15f * B2_PI));
            b2CreatePolygonShape(groundId, &shapeDef, &box);
    ...
    static Sample* Create(SampleContext* context) { return new BodyMove(context); }
...
public:
    ...
    explicit SensorTypes(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new SensorTypes(context); }
...
// This sample shows how to break joints when the internal reaction force becomes large. Instead of
// polling, this uses events.
class JointEvent: public Sample {
public:
    enum { e_count = 6 };

    explicit JointEvent(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new JointEvent(context); }
...
public:
    explicit PersistentContact(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 6.0f};
            m_context->camera.zoom = 7.5f;
        }
    ...
    void Step() override {
        ...
        if (B2_IS_NON_NULL(m_contactId) && b2Contact_IsValid(m_contactId)) {
            ...
            for (int i = 0; i < data.manifold.pointCount; ++i) {
                ...
                DrawWorldString(m_draw, m_camera, p1, b2_colorWhite, "%.2f",
    ...
    static Sample* Create(SampleContext* context) { return new PersistentContact(context); }

    b2ContactId m_contactId;
};

static int samplePersistentContact =
        RegisterSample("Events", "Persistent Contact", PersistentContact::Create);

class SensorHits: public Sample {
public:
    explicit SensorHits(SampleContext* context): Sample(context), m_transforms{} {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new SensorHits(context); }
...
public:
    explicit ProjectileEvent(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void Step() override {
        ...
        for (int i = 0; i < contactEvents.beginCount; ++i) {
            ...
            if (B2_ID_EQUALS(event->shapeIdA, m_projectileShapeId) ||
                B2_ID_EQUALS(event->shapeIdB, m_projectileShapeId)) {
    ...
    static Sample* Create(SampleContext* context) { return new ProjectileEvent(context); }
...
static int sampleProjectileEvent =
        RegisterSample("Events", "Projectile Event", ProjectileEvent::Create);
