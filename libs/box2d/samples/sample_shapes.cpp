...
public:
    enum ShapeType { e_circleShape = 0, e_capsuleShape, e_boxShape };

    explicit ChainShape(SampleContext* context): Sample(context) {
    ...
    void CreateScene() {
        ...
        //				   {-20.58325, 31.7395},  {-11.0089998, 23.2290001},
        //{-8.67700005, 21.15625},
    ...
    static Sample* Create(SampleContext* context) { return new ChainShape(context); }
...
public:
    explicit CompoundShapes(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new CompoundShapes(context); }
...
static int sampleCompoundShape =
        RegisterSample("Shapes", "Compound Shapes", CompoundShapes::Create);
...
public:
    ...
    explicit ShapeFilter(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.zoom = 25.0f * 0.5f;
    ...
    static Sample* Create(SampleContext* context) { return new ShapeFilter(context); }
...
public:
    ...
    explicit CustomFilter(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 5.0f};
    ...
    static Sample* Create(SampleContext* context) { return new CustomFilter(context); }
...
public:
    ...
    explicit Restitution(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new Restitution(context); }
...
public:
    explicit Friction(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new Friction(context); }
};
...
public:
    explicit RollingResistance(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void Step() override {
        ...
        for (int i = 0; i < 20; ++i) {
            DrawWorldString(m_draw, m_camera, {-41.5f, 2.0f * i + 1.0f}, b2_colorWhite, "%.2f",
                            m_resistScale * i);
        }
    }

    static Sample* Create(SampleContext* context) { return new RollingResistance(context); }

    float m_resistScale;
    float m_lift;
};

static int sampleRollingResistance =
        RegisterSample("Shapes", "Rolling Resistance", RollingResistance::Create);

class ConveyorBelt: public Sample {
public:
    explicit ConveyorBelt(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new ConveyorBelt(context); }
};
...
public:
    explicit TangentSpeed(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
...

        {
            ...
            // const char* path = "M 613.8334,185.20833 H 500.06255 L 470.95838,182.5625
            // 444.50004,174.625 418.04171,161.39583 "
            //				   "394.2292,140.22917 h "
            //				   "-13.22916 v 44.97916 H 68.791712 V 0 h -21.16671 v 206.375 l
            //566.208398,-1e-5 z";

            const char* path = "m 613.8334,185.20833 -42.33338,0 h -37.04166 l -34.39581,0 "
                               "-29.10417,-2.64583 -26.45834,-7.9375 "
                               "-26.45833,-13.22917 -23.81251,-21.16666 h -13.22916 v 44.97916 H "
                               "68.791712 V 0 h -21.16671 v "
    ...
    static Sample* Create(SampleContext* context) { return new TangentSpeed(context); }
...
public:
    explicit ModifyGeometry(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new ModifyGeometry(context); }
...
static int sampleModifyGeometry =
        RegisterSample("Shapes", "Modify Geometry", ModifyGeometry::Create);

// Shows how to link to chain shapes together. This is a useful technique for building large game
// levels with smooth collision.
class ChainLink: public Sample {
public:
    explicit ChainLink(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 5.0f};
            m_context->camera.zoom = 25.0f * 0.5f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new ChainLink(context); }
};
...
public:
    explicit RoundedShapes(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new RoundedShapes(context); }
};
...
public:
    explicit EllipseShape(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.zoom = 25.0f * 0.55f;
            m_context->camera.center = {2.0f, 8.0f};
        }
        ...
        b2Vec2 points[6] = {
                {0.0f, -0.25f},   {0.0f, 0.25f},    {0.05f, 0.075f},
                {-0.05f, 0.075f}, {0.05f, -0.075f}, {-0.05f, -0.075f},
        };
    ...
    static Sample* Create(SampleContext* context) { return new EllipseShape(context); }
};
...
public:
    explicit OffsetShapes(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.zoom = 25.0f * 0.55f;
            m_context->camera.center = {2.0f, 8.0f};
        }
    ...
    static Sample* Create(SampleContext* context) { return new OffsetShapes(context); }
};
...
public:
    explicit Explosion(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 0.0f};
    ...
    void Step() override {
        if (m_context->pause == false || m_context->singleStep == true) {
            m_referenceAngle +=
                    m_context->hertz > 0.0f ? 60.0f * B2_PI / 180.0f / m_context->hertz : 0.0f;
    ...
    static Sample* Create(SampleContext* context) { return new Explosion(context); }
...
public:
    explicit RecreateStatic(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new RecreateStatic(context); }
...
public:
    explicit BoxRestitution(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 5.0f};
            m_context->camera.zoom = 10.0f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new BoxRestitution(context); }

    static constexpr int m_count = 10;
};

static int sampleBoxRestitution =
        RegisterSample("Shapes", "Box Restitution", BoxRestitution::Create);

class Wind: public Sample {
public:
    enum ShapeType { e_circleShape = 0, e_capsuleShape, e_boxShape };

    explicit Wind(SampleContext* context): Sample(context), m_bodyIds{} {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new Wind(context); }
