...
public:
    explicit LargeWorld(SampleContext* context): Sample(context) {
        m_period = 40.0f;
...

        {
            ...
            for (int i = 0; i < m_gridCount; ++i) {
                ...
                // Most algorithms in Box2D work in local coordinates, but contact points are
                // computed relative to the body origin. This makes a noticeable improvement in
                // stability far from the origin.
                if (i % 10 == 0) {
                ...
                for (int j = 0; j < ycount; ++j) {
                    b2Polygon square = b2MakeOffsetBox(0.4f * m_gridSize, 0.4f * m_gridSize,
                                                       {xShape, y}, b2Rot_identity);
                    square.radius = 0.1f;
        ...
        for (int cycleIndex = 0; cycleIndex < m_cycleCount; ++cycleIndex) {
            ...
            if (remainder == 0) {
                ...
                for (int i = 0; i < 5; ++i) {
                    Human human = {};
                    CreateHuman(&human, m_worldId, position, 1.5f, 0.05f, 0.0f, 0.0f,
                                humanIndex + 1, nullptr, false);
                    humanIndex += 1;
                    position.x += 1.0f;
                }
            } else {
    ...
    static Sample* Create(SampleContext* context) { return new LargeWorld(context); }
