...
static float b2CastResult_Closest(b2ShapeId shapeId, b2Vec2 point, b2Vec2 normal, float fraction,
                                  void* c) {
...
public:
    explicit ShapeCastChain(SampleContext* context): Sample(context) {
        // World Body & Shape
    ...
    void Step() override {
        ...
        const b2ShapeProxy shapeProxy =
                b2MakeProxy(characterBox_.vertices, characterBox_.count, characterBox_.radius);
    ...
    bool ShapeCastSingle(PhysicsHitQueryResult2D& outResult, b2Vec2 start, b2Vec2 end,
                         float rotation, const b2ShapeProxy& shape) const {
        ...
        b2World_CastShape(m_worldId, &transformedShape, translation, filter, &b2CastResult_Closest,
                          &context);
    ...
    static Sample* Create(SampleContext* context) { return new ShapeCastChain(context); }
...
static int sampleShapeCastChain =
        RegisterSample("Issues", "Shape Cast Chain", ShapeCastChain::Create);

class BadSteiner: public Sample {
public:
    explicit BadSteiner(SampleContext* context): Sample(context) {
    ...
    static Sample* Create(SampleContext* context) { return new BadSteiner(context); }
};
...
public:
    explicit DisableCrash(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new DisableCrash(context); }
...
public:
    explicit Crash01(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.8f, 6.4f};
            m_context->camera.zoom = 25.0f * 0.4f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new Crash01(context); }
...
public:
    explicit StaticVsBulletBug(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new StaticVsBulletBug(context); }
};

static int staticVsBulletBug =
        RegisterSample("Issues", "StaticVsBulletBug", StaticVsBulletBug::Create);

// This simulations stresses the solver by putting a light mass between two bodies on a prismatic
// joint with a stiff spring. This can be made stable by increasing the size of the middle circle
// and/or increasing the number of sub-steps.
class UnstablePrismaticJoints: public Sample {
public:
    explicit UnstablePrismaticJoints(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 1.75f};
    ...
    static Sample* Create(SampleContext* context) { return new UnstablePrismaticJoints(context); }
};
...
public:
    explicit UnstableWindmill(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 1.75f};
            m_context->camera.zoom = 32.0f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new UnstableWindmill(context); }
};

static int sampleUnstableWindmill =
        RegisterSample("Issues", "Unstable Windmill", UnstableWindmill::Create);
