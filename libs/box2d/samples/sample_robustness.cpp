...
public:
    explicit HighMassRatio1(SampleContext* context): Sample(context) {
    ...
    static Sample* Create(SampleContext* context) { return new HighMassRatio1(context); }
};

static int sampleHighMassRatio1 =
        RegisterSample("Robustness", "HighMassRatio1", HighMassRatio1::Create);

// Big box on small boxes
class HighMassRatio2: public Sample {
public:
    explicit HighMassRatio2(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new HighMassRatio2(context); }
};

static int sampleHighMassRatio2 =
        RegisterSample("Robustness", "HighMassRatio2", HighMassRatio2::Create);

// Big box on small triangles
class HighMassRatio3: public Sample {
public:
    explicit HighMassRatio3(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 16.5f};
            m_context->camera.zoom = 25.0f;
        }
...

        {
            ...
            float extent = 1.0f;
            b2Vec2 points[3] = {
                    {-0.5f * extent, 0.0f}, {0.5f * extent, 0.0f}, {0.0f, 1.0f * extent}};
    ...
    static Sample* Create(SampleContext* context) { return new HighMassRatio3(context); }
};

static int sampleHighMassRatio3 =
        RegisterSample("Robustness", "HighMassRatio3", HighMassRatio3::Create);

class OverlapRecovery: public Sample {
public:
    explicit OverlapRecovery(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void UpdateGui() override {
        ...
        changed = changed ||
                  ImGui::SliderFloat("Damping Ratio", &m_dampingRatio, 0.0f, 20.0f, "%.1f");
    ...
    static Sample* Create(SampleContext* context) { return new OverlapRecovery(context); }
...
static int sampleOverlapRecovery =
        RegisterSample("Robustness", "Overlap Recovery", OverlapRecovery::Create);

class TinyPyramid: public Sample {
public:
    explicit TinyPyramid(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
...

        {
            ...
            for (int i = 0; i < baseCount; ++i) {
                ...
                for (int j = i; j < baseCount; ++j) {
                    float x = (i + 1.0f) * m_extent + 2.0f * (j - i) * m_extent -
                              baseCount * m_extent;
                    bodyDef.position = {x, y};
    ...
    static Sample* Create(SampleContext* context) { return new TinyPyramid(context); }
...
public:
    explicit Cart(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void UpdateGui() override {
        ...
        ImGui::Text("Contact");
        changed = changed ||
                  ImGui::SliderFloat("Hertz##contact", &m_contactHertz, 0.0f, 240.0f, "%.f");
        changed = changed || ImGui::SliderFloat("Damping Ratio##contact", &m_contactDampingRatio,
                                                0.0f, 100.0f, "%.f");
        changed = changed || ImGui::SliderFloat("Speed", &m_contactSpeed, 0.0f, 5.0f, "%.1f");

        if (changed) {
            b2World_SetContactTuning(m_worldId, m_contactHertz, m_contactDampingRatio,
                                     m_contactSpeed);
            CreateScene();
        }
        ...
        ImGui::Text("Joint");
        changed = changed ||
                  ImGui::SliderFloat("Hertz##joint", &m_constraintHertz, 0.0f, 240.0f, "%.f");
        changed = changed || ImGui::SliderFloat("Damping Ratio##joint", &m_constraintDampingRatio,
                                                0.0f, 20.0f, "%.f");
    ...
    static Sample* Create(SampleContext* context) { return new Cart(context); }
...
public:
    explicit MultiplePrismatic(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new MultiplePrismatic(context); }
};

static int sampleMultiplePrismatic =
        RegisterSample("Robustness", "Multiple Prismatic", MultiplePrismatic::Create);
