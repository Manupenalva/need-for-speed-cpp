...
public:
    ...
    explicit BounceHouse(SampleContext* context): Sample(context) {
    ...
    static Sample* Create(SampleContext* context) { return new BounceHouse(context); }
...
public:
    explicit BounceHumans(SampleContext* context): Sample(context) {
        m_context->camera.center = {0.0f, 0.0f};
    ...
    void Step() override {
        if (m_humanCount < 5 && m_countDown <= 0.0f) {
            ...
            CreateHuman(m_humans + m_humanCount, m_worldId, {0.0f, 5.0f}, 1.0f, jointFrictionTorque,
                        jointHertz, jointDampingRatio, 1, nullptr, true);
            // Human_SetVelocity( m_humans + m_humanCount, { 10.0f - 5.0f * m_humanCount, -20.0f
            // + 5.0f * m_humanCount } );
    ...
    static Sample* Create(SampleContext* context) { return new BounceHumans(context); }
...
public:
    explicit ChainDrop(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 0.0f};
    ...
    static Sample* Create(SampleContext* context) { return new ChainDrop(context); }
...
public:
    explicit ChainSlide(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new ChainSlide(context); }
};
...
public:
    explicit SegmentSlide(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 10.0f};
            m_context->camera.zoom = 15.0f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new SegmentSlide(context); }
};
...
public:
    explicit SkinnyBox(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new SkinnyBox(context); }
...
public:
    ...
    explicit GhostBumps(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void CreateScene() {
        ...
        if (m_useChain) {
            ...
            points[7] = b2Add(points[6], {2.0f * hx * m + 2.0f * hy * (1.0f - m),
            ...
            points[11] = b2Add(points[10], {2.0f * hx * m + 2.0f * hy * (1.0f - m),
                                            2.0f * hx * m + 2.0f * hy * (1.0f - m)});
    ...
    static Sample* Create(SampleContext* context) { return new GhostBumps(context); }
...
public:
    explicit SpeculativeFallback(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {1.0f, 5.0f};
            m_context->camera.zoom = 25.0f * 0.25f;
        }

        {
            ...
            b2Vec2 points[5] = {
                    {-2.0f, 4.0f}, {2.0f, 4.0f}, {2.0f, 4.1f}, {-0.5f, 4.2f}, {-2.0f, 4.2f}};
    ...
    static Sample* Create(SampleContext* context) { return new SpeculativeFallback(context); }
};
...
public:
    explicit SpeculativeSliver(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new SpeculativeSliver(context); }
};

static int sampleSpeculativeSliver =
        RegisterSample("Continuous", "Speculative Sliver", SpeculativeSliver::Create);

// This shows that while Box2D uses speculative collision, it does not lead to speculative ghost
// collisions at small distances
class SpeculativeGhost: public Sample {
public:
    explicit SpeculativeGhost(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {0.0f, 1.75f};
...

        {
            ...
            bodyDef.linearVelocity = {0.1f * 1.25f * m_context->hertz,
                                      -0.1f * 1.25f * m_context->hertz};
            bodyDef.gravityScale = 0.0f;
            b2BodyId bodyId = b2CreateBody(m_worldId, &bodyDef);
    ...
    static Sample* Create(SampleContext* context) { return new SpeculativeGhost(context); }
};

static int sampleSpeculativeGhost =
        RegisterSample("Continuous", "Speculative Ghost", SpeculativeGhost::Create);

// This shows that Box2D does not have pixel perfect collision.
class PixelImperfect: public Sample {
public:
    explicit PixelImperfect(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
...

        {
            ...
            b2Polygon ballShape = b2MakeRoundedBox(4.0f / pixelsPerMeter, 4.0f / pixelsPerMeter,
                                                   0.9f / pixelsPerMeter);
    ...
    static Sample* Create(SampleContext* context) { return new PixelImperfect(context); }

    b2BodyId m_ballId;
};

static int samplePixelImperfect =
        RegisterSample("Continuous", "Pixel Imperfect", PixelImperfect::Create);

class RestitutionThreshold: public Sample {
public:
    explicit RestitutionThreshold(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
            m_context->camera.center = {7.0f, 5.0f};
            m_context->camera.zoom = 6.0f;
        }
    ...
    static Sample* Create(SampleContext* context) { return new RestitutionThreshold(context); }
...
public:
    explicit Drop(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void Scene3() {
        ...
        CreateHuman(&m_human, m_worldId, {0.0f, 40.0f}, 1.0f, jointFrictionTorque, jointHertz,
    ...
    static Sample* Create(SampleContext* context) { return new Drop(context); }
...
public:
    explicit Pinball(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
        ...
        b2BodyId groundId = {};
        {
            ...
            b2Vec2 vs[5] = {
                    {-8.0f, 6.0f}, {-8.0f, 20.0f}, {8.0f, 20.0f}, {8.0f, 6.0f}, {0.0f, -2.0f}};
    ...
    static Sample* Create(SampleContext* context) { return new Pinball(context); }
...
public:
    explicit Wedge(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new Wedge(context); }
};
