...
void Sample::ResetText() { m_textLine = m_textIncrement; }
...
void Sample::Step() {
    ...
    if (m_context->drawCounters) {
        ...
        DrawTextLine("bodies/shapes/contacts/joints = %d/%d/%d/%d", s.bodyCount, s.shapeCount,
    ...
    // Track maximum profile times
    {
        ...
        m_maxProfile.solveConstraints =
                b2MaxFloat(m_maxProfile.solveConstraints, p.solveConstraints);
        m_maxProfile.prepareConstraints =
                b2MaxFloat(m_maxProfile.prepareConstraints, p.prepareConstraints);
        m_maxProfile.integrateVelocities =
                b2MaxFloat(m_maxProfile.integrateVelocities, p.integrateVelocities);
        ...
        m_maxProfile.integratePositions =
                b2MaxFloat(m_maxProfile.integratePositions, p.integratePositions);
        m_maxProfile.relaxImpulses = b2MaxFloat(m_maxProfile.relaxImpulses, p.relaxImpulses);
        m_maxProfile.applyRestitution =
                b2MaxFloat(m_maxProfile.applyRestitution, p.applyRestitution);
    ...
    if (m_context->drawProfile) {
        ...
        DrawTextLine("step [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.step, aveProfile.step,
                     m_maxProfile.step);
        DrawTextLine("pairs [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.pairs, aveProfile.pairs,
                     m_maxProfile.pairs);
        DrawTextLine("collide [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.collide, aveProfile.collide,
                     m_maxProfile.collide);
        DrawTextLine("solve [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.solve, aveProfile.solve,
                     m_maxProfile.solve);
        DrawTextLine("> prepare tasks [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.prepareStages,
        ...
        DrawTextLine(">> prepare constraints [ave] (max) = %5.2f [%6.2f] (%6.2f)",
                     p.prepareConstraints, aveProfile.prepareConstraints,
                     m_maxProfile.prepareConstraints);
        DrawTextLine(">> integrate velocities [ave] (max) = %5.2f [%6.2f] (%6.2f)",
                     p.integrateVelocities, aveProfile.integrateVelocities,
                     m_maxProfile.integrateVelocities);
        DrawTextLine(">> warm start [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.warmStart,
                     aveProfile.warmStart, m_maxProfile.warmStart);
        DrawTextLine(">> solve impulses [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.solveImpulses,
                     aveProfile.solveImpulses, m_maxProfile.solveImpulses);
        DrawTextLine(">> integrate positions [ave] (max) = %5.2f [%6.2f] (%6.2f)",
                     p.integratePositions, aveProfile.integratePositions,
                     m_maxProfile.integratePositions);
        DrawTextLine(">> relax impulses [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.relaxImpulses,
        ...
        DrawTextLine(">> store impulses [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.storeImpulses,
                     aveProfile.storeImpulses, m_maxProfile.storeImpulses);
        DrawTextLine(">> split islands [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.splitIslands,
                     aveProfile.splitIslands, m_maxProfile.splitIslands);
        DrawTextLine("> update transforms [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.transforms,
                     aveProfile.transforms, m_maxProfile.transforms);
        DrawTextLine("> joint events [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.jointEvents,
                     aveProfile.jointEvents, m_maxProfile.jointEvents);
        DrawTextLine("> hit events [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.hitEvents,
                     aveProfile.hitEvents, m_maxProfile.hitEvents);
        DrawTextLine("> refit BVH [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.refit, aveProfile.refit,
                     m_maxProfile.refit);
        DrawTextLine("> sleep islands [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.sleepIslands,
...
// "M 47.625004,185.20833 H 161.39585
// l 29.10417,-2.64583 26.45834,-7.9375 26.45833,-13.22917 23.81251,-21.16666 h " "13.22916
// v 44.97916 H 592.66669 V 0 h 21.16671 v 206.375 l -566.208398,-1e-5 z"
int Sample::ParsePath(const char* svgPath, b2Vec2 offset, b2Vec2* points, int capacity, float scale,
                      bool reverseOrder) {
    int pointCount = 0;
    b2Vec2 currentPoint = {};
    ...
    while (*ptr != '\0') {
        ...
        points[pointCount] = {scale * (currentPoint.x + offset.x),
