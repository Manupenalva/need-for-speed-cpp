...
void Car::Spawn(b2WorldId worldId, b2Vec2 position, float scale, float hertz, float dampingRatio,
                float torque, void* userData) {
...
void Truck::Spawn(b2WorldId worldId, b2Vec2 position, float scale, float hertz, float dampingRatio,
                  float torque, float density, void* userData) {
    assert(m_isSpawned == false);
    ...
    // b2Vec2 vertices[6] = {
    //	{ -1.5f, -0.5f }, { 1.5f, -0.5f }, { 1.5f, 0.0f }, { 0.0f, 0.9f }, { -1.15f, 0.9f }, {
    //-1.5f, 0.2f },
    // };
    ...
    b2Polygon box = b2MakeOffsetBox(1.25f * scale, 0.1f * scale, {-2.05f * scale, -0.275f * scale},
                                    b2Rot_identity);
    box.radius = 0.1f * scale;
    b2CreatePolygonShape(m_chassisId, &shapeDef, &box);

    box = b2MakeOffsetBox(0.05f * scale, 0.35f * scale, {-3.25f * scale, 0.375f * scale},
                          b2Rot_identity);
    box.radius = 0.1f * scale;
    b2CreatePolygonShape(m_chassisId, &shapeDef, &box);
