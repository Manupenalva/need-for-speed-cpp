...
public:
    explicit FallingHinges(SampleContext* context): Sample(context) {
    ...
    static Sample* Create(SampleContext* context) { return new FallingHinges(context); }

    FallingHingeData m_data;
    bool m_done;
};

static int sampleFallingHinges =
        RegisterSample("Determinism", "Falling Hinges", FallingHinges::Create);
