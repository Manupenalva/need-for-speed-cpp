...
public:
    enum ShapeType { e_point, e_segment, e_triangle, e_box };

    explicit ShapeDistance(SampleContext* context): Sample(context) {
    ...
    void DrawShape(ShapeType type, b2Transform transform, float radius, b2HexColor color) {
        switch (type) {
            ...
            case e_triangle:
                DrawSolidPolygon(m_draw, transform, m_triangle.vertices, m_triangle.count, radius,
                                 color);
                break;
    ...
    void UpdateGui() override {
        ...
        ImGui::Begin("Shape Distance", nullptr,
    ...
    void Step() override {
        ...
        if (m_cache.count == 1) {
            ...
            DrawTextLine("cache = {%d, %d}, {%d, %d}", m_cache.indexA[0], m_cache.indexA[1],
                         m_cache.indexB[0], m_cache.indexB[1]);
        } else if (m_cache.count == 3) {
            DrawTextLine("cache = {%d, %d, %d}, {%d, %d, %d}", m_cache.indexA[0], m_cache.indexA[1],
                         m_cache.indexA[2], m_cache.indexB[0], m_cache.indexB[1],
                         m_cache.indexB[2]);
        }
    }

    static Sample* Create(SampleContext* context) { return new ShapeDistance(context); }
...
static int sampleShapeDistance =
        RegisterSample("Collision", "Shape Distance", ShapeDistance::Create);
...
static float RayCallback(const b2RayCastInput* input, int32_t proxyId, uint64_t userData,
                         void* context);
...
public:
    explicit DynamicTree(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
        ...
        for (int i = 0; i < m_rowCount; ++i) {
            ...
            for (int j = 0; j < m_columnCount; ++j) {
                ...
                if (fillTest <= m_fill) {
                    ...
                    p->proxyId = b2DynamicTree_CreateProxy(&m_tree, p->fatBox,
                                                           B2_DEFAULT_CATEGORY_BITS, m_proxyCount);
                    p->rayStamp = -1;
                    p->queryStamp = -1;
                    p->moved = false;
                    ++m_proxyCount;
    ...
    void Step() override {
        ...
        if (m_rayDrag) {
            ...
            b2TreeStats result =
                    b2DynamicTree_RayCast(&m_tree, &input, B2_DEFAULT_MASK_BITS, RayCallback, this);
            ...
            DrawTextLine("node visits = %d, leaf visits = %d", result.nodeVisits,
                         result.leafVisits);
        }
        ...
        DrawTextLine("proxies = %d, height = %d, hmin = %d, area ratio = %.1f", m_proxyCount,
    ...
    static Sample* Create(SampleContext* context) { return new DynamicTree(context); }
...
static float RayCallback(const b2RayCastInput* input, int proxyId, uint64_t userData,
                         void* context) {
    DynamicTree* sample = static_cast<DynamicTree*>(context);
    Proxy* proxy = sample->m_proxies + userData;
    assert(proxy->proxyId == proxyId);
...
public:
    explicit RayCast(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void Step() override {
        ...
        {
            b2Transform transform = {b2Add(m_transform.p, offset), m_transform.q};
            DrawSolidPolygon(m_draw, transform, m_triangle.vertices, m_triangle.count, 0.0f,
                             color1);
    ...
    static Sample* Create(SampleContext* context) { return new RayCast(context); }
...
static float RayCastClosestCallback(b2ShapeId shapeId, b2Vec2 point, b2Vec2 normal, float fraction,
                                    void* context) {
...
static float RayCastAnyCallback(b2ShapeId shapeId, b2Vec2 point, b2Vec2 normal, float fraction,
                                void* context) {
    CastContext* rayContext = (CastContext*)context;
...
static float RayCastMultipleCallback(b2ShapeId shapeId, b2Vec2 point, b2Vec2 normal, float fraction,
                                     void* context) {
    CastContext* rayContext = (CastContext*)context;
...
static float RayCastSortedCallback(b2ShapeId shapeId, b2Vec2 point, b2Vec2 normal, float fraction,
                                   void* context) {
    CastContext* rayContext = (CastContext*)context;
    ...
    if (index == 3) {
        // not closer, continue but tell the caller not to consider fractions further than the
        // largest fraction acquired this only happens once the buffer is full
...
public:
    enum Mode { e_any = 0, e_closest = 1, e_multiple = 2, e_sorted = 3 };

    enum CastType { e_rayCast = 0, e_circleCast = 1, e_capsuleCast = 2, e_polygonCast = 3 };

    enum { e_maxCount = 64 };

    explicit CastWorld(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
...

        {
            ...
            b2Vec2 vertices[8] = {{0.5f * s, 0.0f}, {0.5f * w, b},    {0.5f * w, b + s},
                                  {0.5f * s, w},    {-0.5f * s, w},   {-0.5f * w, b + s},
                                  {-0.5f * w, b},   {-0.5f * s, 0.0f}};
    ...
    void UpdateGui() override {
        ...
        ImGui::Begin("Ray-cast World", nullptr,
                     ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize);
    ...
    void Step() override {
        ...
        if (m_simple) {
            ...
            b2RayResult result = b2World_CastRayClosest(m_worldId, m_rayStart, rayTranslation,
            ...
            b2Polygon box =
                    b2MakeOffsetRoundedBox(0.125f, 0.25f, transform.p, transform.q, m_castRadius);
            b2ShapeProxy proxy = {};

            if (m_castType == e_rayCast) {
                b2World_CastRay(m_worldId, m_rayStart, rayTranslation, b2DefaultQueryFilter(),
                                modeFcn, &context);
            } else {
                ...
                b2World_CastShape(m_worldId, &proxy, rayTranslation, b2DefaultQueryFilter(),
                                  modeFcn, &context);
            }
            ...
            if (context.count > 0) {
                ...
                for (int i = 0; i < context.count; ++i) {
                    ...
                    if (m_castType == e_circleCast) {
                        ...
                        DrawSolidCircle(m_draw, {center, shiftedTransform.q}, m_castRadius,
                                        b2_colorYellow);
                    } else if (m_castType == e_capsuleCast) {
                        ...
                        DrawSolidPolygon(m_draw, shiftedTransform, box.vertices, box.count,
                                         box.radius, b2_colorYellow);
                    }
                }
            } else {
                DrawLine(m_draw, m_rayStart, m_rayEnd, color2);
                ...
                if (m_castType == e_circleCast) {
                    ...
                    DrawSolidPolygon(m_draw, shiftedTransform, box.vertices, box.count, box.radius,
                                     b2_colorYellow);
                }
            }
        }
    ...
    static Sample* Create(SampleContext* context) { return new CastWorld(context); }
...
public:
    enum { e_circleShape = 0, e_capsuleShape = 1, e_boxShape = 2 };
    ...
    explicit OverlapWorld(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void Step() override {
        ...
        if (m_shapeType == e_circleShape) {
            ...
            proxy = b2MakeProxy(&capsule.center1, 2, capsule.radius);
            DrawSolidCapsule(m_draw, capsule.center1, capsule.center2, capsule.radius,
                             b2_colorWhite);
    ...
    static Sample* Create(SampleContext* context) { return new OverlapWorld(context); }
...
public:
    explicit Manifold(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void Step() override {
        ...
        // rox-rox
        {
            ...
            // b2Transform transform2 = {{5.52862263f, 2.51146317f}, {-0.859374702f,
            // -0.511346340f}};
        ...
        // segment-rox
        {
            ...
            b2Transform transform1 = {offset, b2Rot_identity};
            b2Transform transform2 = {b2Add(m_transform.p, offset), m_transform.q};
            // b2Transform transform2 = {b2Add({-1.44583416f, 0.397352695f}, offset),
            // m_transform.q};
        ...
        // wox-wox
        {
            b2Vec2 p1s[3] = {{0.175740838, 0.224936664},
                             {-0.301293969, 0.194021404},
                             {-0.105151534, -0.432157338}};
            b2Vec2 p2s[3] = {{-0.427884758, -0.225028217},
                             {0.0566576123, -0.128772855},
                             {0.176625848, 0.338923335}};
        ...
        // chain-segment vs circle
        {
            ...
            b2Manifold m =
                    b2CollideChainSegmentAndCircle(&segment, transform1, &circle, transform2);
        ...
        {
            b2ChainSegment segment1 = {
                    {2.0f, 1.0f}, {{1.0f, 1.0f}, {-1.0f, 0.0f}}, {-2.0f, 0.0f}, -1};
            b2ChainSegment segment2 = {
                    {3.0f, 1.0f}, {{2.0f, 1.0f}, {1.0f, 1.0f}}, {-1.0f, 0.0f}, -1};
            // b2ChainSegment segment1 = {{2.0f, 0.0f}, {{1.0f, 0.0f}, {-1.0f, 0.0f}}, {-2.0f,
            // 0.0f}, -1}; b2ChainSegment segment2 = {{3.0f, 0.0f}, {{2.0f, 0.0f}, {1.0f, 0.0f}},
            // {-1.0f, 0.0f}, -1}; b2ChainSegment segment1 = {{0.5f, 1.0f}, {{0.0f, 2.0f},
            // {-0.5f, 1.0f}}, {-1.0f, 0.0f}, -1}; b2ChainSegment segment2 = {{1.0f, 0.0f},
            // {{0.5f, 1.0f}, {0.0f, 2.0f}}, {-0.5f, 1.0f}, -1};
            float h = 0.5f - m_round;
            b2Polygon rox = b2MakeRoundedBox(h, h, m_round);
            ...
            b2Manifold m1 = b2CollideChainSegmentAndPolygon(&segment1, transform1, &rox, transform2,
                                                            &m_smgroxCache1);
            b2Manifold m2 = b2CollideChainSegmentAndPolygon(&segment2, transform1, &rox, transform2,
                                                            &m_smgroxCache2);
        ...
        // chain-segment vs capsule
        {
            ...
            b2Manifold m1 = b2CollideChainSegmentAndCapsule(&segment1, transform1, &capsule,
                                                            transform2, &m_smgcapCache1);
            b2Manifold m2 = b2CollideChainSegmentAndCapsule(&segment2, transform1, &capsule,
                                                            transform2, &m_smgcapCache2);
    ...
    static Sample* Create(SampleContext* context) { return new Manifold(context); }
...
public:
    ...
    explicit SmoothManifold(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void UpdateGui() override {
        ...
        ImGui::Begin("Smooth Manifold", nullptr,
                     ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize);
        ImGui::PushItemWidth(100.0f);
    ...
    void Step() override {
        ...
        if (m_shapeType == e_circleShape) {
            ...
            for (int i = 0; i < m_count; ++i) {
                ...
                b2Manifold m = b2CollideChainSegmentAndPolygon(segment, transform1, &rox,
                                                               transform2, &cache);
                DrawManifold(&m);
            }
        }
    ...
    static Sample* Create(SampleContext* context) { return new SmoothManifold(context); }
...
static int sampleSmoothManifoldIndex =
        RegisterSample("Collision", "Smooth Manifold", SmoothManifold::Create);

class ShapeCast: public Sample {
public:
    enum ShapeType { e_point, e_segment, e_triangle, e_box };

    explicit ShapeCast(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    void Step() override {
        ...
        b2DistanceOutput distanceOutput =
                b2ShapeDistance(&distanceInput, &distanceCache, nullptr, 0);

        DrawTextLine("hit = %s, iterations = %d, fraction = %g, distance = %g",
                     output.hit ? "true" : "false", output.iterations, output.fraction,
        ...
        DrawTextLine("distance = %.2f, iterations = %d", distanceOutput.distance,
                     output.iterations);
    }

    static Sample* Create(SampleContext* context) { return new ShapeCast(context); }
...
public:
    explicit TimeOfImpact(SampleContext* context): Sample(context) {
        if (m_context->restart == false) {
    ...
    static Sample* Create(SampleContext* context) { return new TimeOfImpact(context); }
